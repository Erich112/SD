...
class BidderMicroservice {
    ...
    init {
        try {
            ...
            var NamesList = mutableListOf(
                "Liam", "Olivia", "Noah","Emma","Oliver","Charlotte",
                "James","Amelia","Elijah","Sophia","William","Isabella","Henry","Ava",
                "Lucas","Mia","Benjamin","Evelyn","Theodore","Luna"
            )
            var name = NamesList.random()
            var EmailsList = mutableListOf("@gmail.com", "@yahoo.com", "@hotmail.com","@yandex.ru")
            var email = name+auctioneerSocket.localPort+EmailsList.random()
            var telefon = "+40703" + auctioneerSocket.localPort;
            myIdentity = "[${name}, ${email}, ${telefon}]"
