...
class AgendaController {
    ...
    @Autowired
    private lateinit var _encryptingService: IEncryptingService
    ...
    fun search(
        @RequestParam(required = false, name = "userName", defaultValue = "") userName: String
    ):
            ResponseEntity<List<Person>> {
        val personList = _agendaService.searchAgenda(userName)
    ...
    @RequestMapping(value = ["/agenda-admin"], method = [RequestMethod.GET])
    fun adminSearch(
        @RequestParam(required = true, name = "key", defaultValue = "") key: String,
        @RequestParam(required = false, name = "lastName", defaultValue = "") lastName: String,
        @RequestParam(required = false, name = "firstName", defaultValue = "") firstName: String,
        @RequestParam(required = false, name = "userName", defaultValue = "") userName: String
    ):
            ResponseEntity<out Any> {
        if(_encryptingService.check(key))
        {
            val personList = _agendaService.searchAgendaAdmin(lastName, firstName, userName)
            var httpStatus = HttpStatus.OK
            if (personList.isEmpty()) {
                httpStatus = HttpStatus.NO_CONTENT
            }
